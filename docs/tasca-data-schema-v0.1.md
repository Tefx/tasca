...
`position` is stable zero-based input order. `media_type` is the fixed public value `text/markdown` and is derived rather than stored. `content` must contain non-whitespace Markdown. `byte_size` is the exact positive UTF-8 byte count validated at admission. Name/content/count limits are application-level contracts. The additive migration uses `CREATE TABLE IF NOT EXISTS`; it does not rewrite saying rows and has no down migration. Batch table deletion removes sayings and lets `ON DELETE CASCADE` remove attachments.
