...
Attachment input is `{name, content}`. Names are 1..128 characters, have `.md` or `.markdown` suffix, and contain no surrounding whitespace, path separator, or NUL. Content is valid UTF-8 text and may be empty or whitespace-only. Limits are eight attachments, 256 KiB exact UTF-8 bytes per item, and 1 MiB total attachment content. Attachment names/content do not participate in mentions or saying FTS.
