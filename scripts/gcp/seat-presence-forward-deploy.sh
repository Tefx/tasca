#!/usr/bin/env bash
# purpose: Forward-deploy the exact 0.1.31 Seat-presence wheel to the existing Tasca VM.
# usage: Set RELEASE_WHEEL and RELEASE_SHA256, then run render or apply locally; apply stages this file and invokes activate on the VM.
# effects: apply transfers the exact wheel and atomically changes only tasca.service's ExecStart path. It does not read or write the environment, secrets, SQLite database, Caddy, or firewall.
# requires: A committed clean producer, gcloud access to rda-engineering/asia-southeast1-b/tasca-mcp, and the admitted remote CPython 3.13. TASCA_FORWARD_TEST_ROOT is only for offline fixture tests.
set -euo pipefail
umask 077

readonly RELEASE_VERSION="0.1.31"
readonly PREVIOUS_VERSION="0.1.30"
...
readonly REMOTE_STAGE_DIR="/var/tmp/tasca-seat-presence-0.1.31"
...
fail() {
    printf 'seat-presence forward deploy: %s\n' "$*" >&2
    exit 1
}
...
require_root() {
    ...
    [[ "$EUID" -eq 0 ]] || fail "activate must run as root"
}
...
require_committed_producer() {
    local file
    for file in scripts/gcp/seat-presence-forward-deploy.sh scripts/gcp/verify_viewer_auth_remote.py; do
        git -C "$REPO_ROOT" ls-files --error-unmatch "$file" >/dev/null
    done
    git -C "$REPO_ROOT" diff --quiet -- \
        scripts/gcp/seat-presence-forward-deploy.sh scripts/gcp/verify_viewer_auth_remote.py
...
emit_manifest() {
    python3 - "$WHEEL_NAME" "$RELEASE_SHA256" "$(git -C "$REPO_ROOT" rev-parse --verify HEAD)" \
        "$(file_hash "$SCRIPT_DIR/seat-presence-forward-deploy.sh")" \
        "$(file_hash "$SCRIPT_DIR/verify_viewer_auth_remote.py")" <<'PY'
import json
import sys
...
print(
    json.dumps(
        {
            "release": {"version": "0.1.31", "wheel": wheel, "sha256": digest},
            "target": {
            ...
            "producer": {
                "path": "scripts/gcp/seat-presence-forward-deploy.sh",
                "revision": revision,
            ...
            "python": {"path": "/var/lib/tasca/.local/share/uv/python/cpython-3.13.15-linux-x86_64-gnu/bin/python3.13", "required": "CPython 3.13"},
            "actions": [
                "stage_tracked_producer_and_exact_wheel",
                "install_exact_wheel_in_0_1_31_venv",
            ...
            "preserved": ["tasca.env", "Secret Manager values", "tasca-data SQLite", "Caddy", "firewall", "/opt/tasca/releases/0.1.30"],
        },
        sort_keys=True,
    )
)
...
stage_and_activate() {
    ...
    producer_sha="$(file_hash "$SCRIPT_DIR/seat-presence-forward-deploy.sh")"
    remote_ssh "install -d -m 0700 ${REMOTE_STAGE_DIR}"
    gcloud compute scp --project="$PROJECT_ID" --zone="$ZONE" --quiet \
        "$SCRIPT_DIR/seat-presence-forward-deploy.sh" \
        "${VM}:${REMOTE_STAGE_DIR}/seat-presence-forward-deploy.sh"
    gcloud compute scp --project="$PROJECT_ID" --zone="$ZONE" --quiet \
        "$RELEASE_WHEEL" "${VM}:${REMOTE_STAGE_DIR}/${WHEEL_NAME}"
    remote_ssh "set -eu; actual=\$(sha256sum -- ${REMOTE_STAGE_DIR}/seat-presence-forward-deploy.sh | awk '{print \$1}'); test \"\$actual\" = \"${producer_sha}\"; sudo install -d -o root -g root -m 0755 /usr/local/lib/tasca; sudo install -o root -g root -m 0700 ${REMOTE_STAGE_DIR}/seat-presence-forward-deploy.sh /usr/local/lib/tasca/seat-presence-forward-deploy.sh"
    remote_command="sudo /usr/local/lib/tasca/seat-presence-forward-deploy.sh activate --wheel ${REMOTE_STAGE_DIR}/${WHEEL_NAME} --wheel-sha ${RELEASE_SHA256}"
...
verify_installed_version() {
    ...
    "$python" -c 'from pathlib import Path; import sys; assert Path(sys.argv[1]).is_dir()' "$RELEASE_DIR"
}
...
replacement = original.replace(previous, current)
...
raise SystemExit(payload.get("version") != "0.1.31")
'; then
            return
        fi
    ...
    printf 'seat-presence forward deploy: %s health did not report %s within %s attempts\n' \
        "$label" "$RELEASE_VERSION" "$HEALTH_ATTEMPTS" >&2
    return 1
}
...
restore_after_activation_failure() {
    ...
    printf 'seat-presence forward deploy: activation failed; pre-0.1.31 unit restored\n' >&2
    exit 1
}
...
activate() {
    ...
    verify_installed_version "$python"
    ...
    if ! require_health "$python" "http://127.0.0.1:8000/api/v1/health" "local"; then
        restore_after_activation_failure "$python"
    fi
    if ! require_health "$python" "https://${HTTPS_HOST}/api/v1/health" "HTTPS"; then
        restore_after_activation_failure "$python"
    fi
    printf 'seat-presence forward deploy: exact %s is active; %s remains available\n' \
