...
TOOL_CONTRACTS: Final[tuple[ToolContract, ...]] = (
    ...
    ToolContract(
        ...
        (
            ...
            P(
                "attachments",
                "Optional ordered Markdown objects ({name, content}); max 8, inserted atomically",
                False,
                None,
            ),
        ),
    ...
    ToolContract(
        ...
        "Read complete Markdown attachment bodies by ID (admin-authenticated MCP only).",
        "Accepts 1..8 IDs and returns bodies in request order. Error: NOT_FOUND if any ID is absent.",
        (P("attachment_ids", "List of 1..8 attachment UUIDs", True),),
        f"{TOOLS_SPEC_ANCHOR}/5-3-sayings-messages/tasca-attachment-get",
        "tasca.shell.mcp.entrypoints.attachment_get",
    ),
    ToolContract(
        "table_listen",
        "tasca.table.listen",
